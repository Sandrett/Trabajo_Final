...
app.config['MAIL_USERNAME'] = 'sanmeds.clinica@gmail.com'
app.config['MAIL_PASSWORD'] = 'ClinicaSanMeds12345'
...
'''
Esta función comprueba si el usuario existe.
Si no existe devuelve error, si existe crea la sesión
Dependiendo si es doctor o paciente devuelve una html u otro
'''
...
'''
Función de Login
Se comprueba si las credenciales introducias son las correctas
si se devuelve un usuario, se crea la sesión.

Si el usuario devuelto es admin da error, no se puede logear aquí.
'''
...
'''
Función para registrar usuarios, si el email introducido
ya está en BD devuelve error
Si no, lo inserta en la BD.
'''
...
'''
Función para adjuntar ficheros entre pacientes/doctores.
'''
...
def email():
    ...
    msg = Message(f'El usuario {name} quiere contactar contigo', sender=email, recipients=["sanmeds.clinica@gmail.com"])
    msg.body = f"{message}\n users email:{email}\n {date}"
    mail.send(msg)
    doctors = User.query.filter_by(role='doctor').all()
    return render_template('index.html', doctors=doctors)
...
'''
Función de login para los usuarios administradores
